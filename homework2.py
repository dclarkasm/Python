...
swords=set()
for word in squery:
        if (word.lower() == "or") or (word.lower() == "and"):
                operators.add(word)
        else:
                swords.add(word)
if (len(operators)==1) and ('or' in operators):
        stype='or'
...
for w in word_list:
        for quote in data_list:
                q = quote.find(w)
                if q > 0:
                        found_at[len(found_at)-1].add(q)
        found_at.append(set())  #append an empty set

final = set()
if stype == 'or':
        final = set(found_at[0]|found_at[1])
elif stype == 'and':
        final = set(found_at[0]&found_at[1])
                
for i in final:
        print("Found " + query + " at line " + str(i) + ": ..."+quote[i:i+50], "...")
